...
* [Функциональность системы](#3-Функциональность-системы)
  ...
  * 3.3 [Выход пользователя из системы](#34-Выход-пользователя-из-системы)
...
Новому пользователю необходимо отправить POST-запрос на URL `http://localhost:8002/api/v1/auth/registration`
...
Чтобы пройти аутентификацию, необходимо отправить POST-запрос на URL `http://localhost:8002/api/v1/auth/login`,
...
Пользоваетль отправляет GET-запрос на `http://localhost:8002/api/v1/auth/refresh`,
...
Для выхода из системы пользователю необходимо отправить POST-запрос на URL `http://localhost:8002/api/v1/auth/logout`.
...
Чтобы выйти со всех устройств, необходимо добавить соотвествующий параметр. URL POST-запроса будет выглядеть так:
`http://localhost:8002/api/v1/auth/logout?fromAllDevices=true`. Все *Refresh-* токены
